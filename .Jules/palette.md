...
## 2025-06-23 - Interactive Element Invisible Keyboard Traps & Fallbacks\n**Learning:** Relying solely on opacity or translateY to visually hide absolute/fixed interactive elements (like ScrollToTop buttons or dynamic tabs) creates invisible keyboard traps where screen reader/keyboard users can tab into off-screen elements. Furthermore, icon-only interactive elements often fail robustly without both `aria-label` and native `title` (dual-pattern fallback), while decorative inner SVGs cause screen reader noise if missing `aria-hidden="true"`.\n**Action:** Always conditionally bind `tabIndex={isVisible ? 0 : -1}` and `aria-hidden={!isVisible}` to elements transitioning in/out visually. Default to adding `title` alongside `aria-label` for icon-only links, explicitly appending `aria-hidden="true"` to their internal Lucide/SVG children.

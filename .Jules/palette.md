...
## 2024-05-27 - [Semantic Structure and Focus Management]
**Learning:** Forcefully blurring an input when cleared via 'Escape' breaks the keyboard navigation flow. Additionally, visually styled lists (like grids of tenets) using `<div>` elements are not announced as lists by screen readers unless explicit roles are added. Finally, copy-to-clipboard interactions need a dedicated `aria-live` announcement region, as simply swapping a button's `aria-label` often isn't read out reliably mid-interaction.
**Action:** When implementing 'Clear' functionality (via button or Escape key), explicitly return or preserve focus on the input (`inputRef.current?.focus()`). Always apply `role="list"` and `role="listitem"` to non-semantic list layouts. Use visually hidden `aria-live="polite"` regions for dynamic clipboard success announcements.

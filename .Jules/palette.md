...
## $(date +%Y-%m-%d) - Preloader considerations for automated testing
**Learning:** The project implements a robust Neobrutalist preloader and entrance animation sequence (`<div className="animate-page-in">` and `<Preloader />`). Automated verification tools like Playwright will fail to locate or interact with elements immediately upon page load if they do not explicitly wait for this sequence to complete.
**Action:** When writing Playwright tests or verification scripts for this project, always implement a wait mechanism for the preloader to clear (e.g., waiting for the main content to become visible or including a short `time.sleep()`) before attempting to interact with the DOM or taking screenshots.

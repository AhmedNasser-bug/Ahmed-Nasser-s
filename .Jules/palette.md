...
## 2026-06-25 - [Accessible Structural Implementations for SPA]
**Learning:** Adding robust accessibility to single page applications requires systematic attention to dynamic states. Using `aria-current="page"` clarifies React Router links; `role="progressbar"` +  correctly exposes custom UI preloaders; and wrapping smooth scrollers in `window.matchMedia('(prefers-reduced-motion: reduce)')` prevents vestibular triggers. Additionally, providing dual patterns (aria-label + title) ensures screen reader and visual tooltip parity.
**Action:** When auditing React SPAs, systematically apply ARIA semantics to dynamic elements (preloaders, routers), always enforce `prefers-reduced-motion` checks on scrollers, and use the dual pattern for icon-only components.
## 2026-06-25 - [Accessible Structural Implementations for SPA]
**Learning:** Adding robust accessibility to single page applications requires systematic attention to dynamic states. Using `aria-current="page"` clarifies React Router links; `role="progressbar"` + aria-valuenow correctly exposes custom UI preloaders; and wrapping smooth scrollers in `window.matchMedia('(prefers-reduced-motion: reduce)')` prevents vestibular triggers. Additionally, providing dual patterns (aria-label + title) ensures screen reader and visual tooltip parity.
**Action:** When auditing React SPAs, systematically apply ARIA semantics to dynamic elements (preloaders, routers), always enforce `prefers-reduced-motion` checks on scrollers, and use the dual pattern for icon-only components.

...
## 2026-05-27 - [Semantic Structure and Cursor Indicators]
**Learning:** Placing standard layout components (like `<Header>` and `<Footer>`) inside the `<main>` tag breaks the semantic expectation for screen readers that `<main>` only contains the primary page content. Furthermore, applying `cursor-pointer` to non-interactive container elements (like decorative block articles that contain separate nested links) misleads users into expecting the entire block to be clickable, degrading UX.
**Action:** Strictly separate global layout components from routing content, ensuring `<main>` wraps only the dynamic page views. Always remove generic `cursor-pointer` utility classes from elements that lack native click handlers or semantic `role="button"` designations.

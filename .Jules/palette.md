...
## 2024-05-18 - [Accessibility & Micro-UX Refinement]
**Learning:** Found that combining visually hidden skip-to-content links with specific dynamic `aria-hidden` and `tabIndex` attributes on conditionally-rendered sticky elements (like scroll-to-top) is vital to avoid invisible keyboard traps. Also reinforced dual-pattern standard for icon-only external links: `aria-label` and `title` must both warn of opening in a new tab.
**Action:** Always verify keyboard focus flow for all off-canvas or conditionally visible elements using `tabIndex={isVisible ? 0 : -1}` and `aria-hidden={!isVisible}`. Enforce a standard that decorative icons get `aria-hidden="true"` automatically if they sit beside readable text or are within a named interactive container.

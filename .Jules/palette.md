...
## 2026-05-19 - [Accessible Project Search and Filters]
**Learning:** For interactive search and filter components, using a mix of visually hidden `<label>`s, functional `aria-pressed` toggles, dynamic clear buttons, and accessible keyboard shortcuts (`/` with `<kbd>` hints) dramatically improves both standard and assistive technology user experiences. Also, when components have an empty state (e.g. no search results), failing to provide a clear path back out (like a "Clear Filters" button that resets state correctly) traps the user in a dead end.
**Action:** Always implement robust empty states with functional escape paths. When building custom toggle filters or tabs, dynamically set and toggle the `aria-pressed="true/false"` attribute so screen readers correctly announce the active state. Prefer providing a semantically linked `<label>` (which can be visually hidden) over solely relying on `aria-label` for standard form inputs (e.g., search bars) for more robust screen reader support.

...
## 2025-03-04 - [The External Link Dilemma & Systemic Focus Visibility]
**Learning:** Adding `aria-label` alone to external links (e.g. social icons) makes it accessible for screen readers but hides the context from sighted mouse users. Furthermore, relying entirely on custom hard-shadow themes for visual interactivity often causes custom buttons (like `ScrollToTop`) to omit explicit `focus-visible` styling, degrading keyboard navigation. Also, using purely decorative SVG background strings creates unintended noise for screen readers if not actively muted with `aria-hidden="true"`.
**Action:** Always pair `aria-label` with a matching `title` attribute for ambiguous or icon-only external links. Ensure every custom interactive component inherits the project's standardized `focus-visible` CSS utilities. Consistently apply `aria-hidden="true"` to structural background grids or visual textures.

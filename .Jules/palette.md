...
## 2024-06-18 - Prevent cursor-pointer misapplication and reinforce dual label pattern
**Learning:** Found widespread misapplication of `cursor-pointer` utility classes on structural, non-interactive `<article>` and `<div>` cards within grids and sections (e.g., in projects and contact). While intended to look like a clickable component visually, it severely degrades UX and accessibility by misleading users and creating false affordances when elements are not focusable or actually clickable. Additionally, some purely icon-based external links lacked a fallback `title` attribute for tooltips, which benefits mouse users without screen readers who need to know what the icon represents.
**Action:** Remove `cursor-pointer` from all non-interactive containers. Consistently verify that elements sporting interactive cursor styles actually utilize correct `<button>`, `<a>`, or `role="button"` syntax with attached actions. Also, routinely provide both `aria-label` and an identical `title` attribute on ambiguous icon buttons (e.g., social links, prev/next carousel navigation).

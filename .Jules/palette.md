...
## 2024-11-21 - [A11y/UX] Foundational Accessibility Audit Implementations

**Learning:** When retrofitting foundational accessibility (like `aria-current`, `skip-to-content`, `prefers-reduced-motion` conditionals, and progress roles) onto an existing Neobrutalist design system built with Vite and Tailwind, small semantic details yield high impact without breaking aesthetic integrity. The use of visually hidden elements for keyboard focus traps (e.g. `ScrollToTop` floating action buttons that animate in based on scroll) must conditionally apply `tabIndex` alongside opacity checks to avoid capturing focus off-screen.

**Action:** Consistently enforce dual-pattern attributes (`aria-label` and `title` together) for all icon-only action links (like external socials or internal carousels). Implement explicit ARIA bounds (`aria-valuenow`, `aria-valuemin`, `aria-valuemax`, `role="progressbar"`) for purely visual UI loading elements. Dynamically sync `aria-current` with React Router's location state across primary navigation links. When using Lenis scroll or heavy GSAP animations, strictly wrap initialization logic in a `prefers-reduced-motion` check for inclusive kinetic design.

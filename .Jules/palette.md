...
## 2024-05-18 - Semantic HTML & Focus Management for External Link Cards
**Learning:** When transforming static grid `div` elements into interactive cards that wrap external links (like GitHub links), the combination of using an `<article>` tag and applying `focus-visible` styling directly to the internal `<a>` tag (rather than adding `tabIndex` to the parent wrapper) creates a significantly cleaner screen reader experience. It prevents screen readers from redundantly announcing the card wrapper and its contents as a single complex interactive element, while still providing a smooth visual focus state for keyboard users.
**Action:** In future implementations of "cards as links", always wrap the structural content in semantic HTML (e.g., `<article>` or `<li>`), avoid `cursor-pointer` on the parent, and ensure all interaction and focus attributes strictly belong to the underlying anchor (`<a>`) tag.

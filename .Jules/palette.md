...
## 2026-06-24 - [React Accessibility Enhancements]
**Learning:** Implementing the dual pattern (title + aria-label) on icon-only links improves UX for both sighted users relying on tooltips and screen reader users. Also, correctly defining interactive elements like carousels using ARIA tablists and progress bars with their respective attributes ensures they are parsed correctly by assistive technologies. Finally, a skip-to-content link must target an element with tabIndex={-1} for programmatic focus to work effectively in React.
**Action:** Always implement 'role=tablist' for carousels, apply 'aria-current="page"' on active navigation, and ensure skip-to-content links target a container capable of receiving programmatic focus.

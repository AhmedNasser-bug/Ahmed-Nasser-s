...
## 2024-05-19 - [Accessible Active Navigation & Semantic Progress States]
**Learning:** React Router `location.pathname` checks can be easily leveraged to conditionally apply `aria-current="page"` to top-level navigation, drastically improving screen reader context without adding external dependencies. Additionally, decorative Three.js/GSAP entrance animations must always have a fallback state or pause mapping to respect OS-level 'prefers-reduced-motion'.
**Action:** Always map custom preloader UI components to `role="progressbar"` with `aria-value*` attributes. Always add `aria-current="page"` or `aria-current="step"` to active interactive navigation elements. And safely query `window.matchMedia('(prefers-reduced-motion: reduce)')` inside a `setTimeout` within `useEffect` to safely bridge SSR/CSR contexts without hydration mismatches.

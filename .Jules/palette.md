...
## 2026-05-27 - [Progress Bar A11y]
**Learning:** Custom preloader or progress bars must be explicitly marked with the `role="progressbar"` attribute and their progress bound to `aria-valuenow` to effectively communicate loading to screen readers.
**Action:** Standardize appending `role="progressbar"`, `aria-valuenow`, `aria-valuemin`, and `aria-valuemax` to all visual progress components.

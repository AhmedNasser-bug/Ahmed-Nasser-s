...
## 2024-05-19 - Semantic HTML and Screen Reader Bypasses
**Learning:** For a single-page application heavily relying on `react-router-dom` and a complex layout, ensuring an accessible `#main-content` area and a "Skip to main content" link is a critical yet often overlooked pattern. Replacing wrapper `<main>` tags that enclose navbars with generic `<div>` tags ensures that standard ARIA landmarks are respected and that the structural hierarchy aligns with expected skip-navigation patterns.
**Action:** When implementing structural application wrappers, actively check whether `<main>` wraps the entire page layout (including nav/footer) or strictly the primary routing content. Apply visually hidden, absolute-positioned skip links focusing semantic `<main id="main-content" tabIndex={-1}>` elements to bypass repetitive navigation.

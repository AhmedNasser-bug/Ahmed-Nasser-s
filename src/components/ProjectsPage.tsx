...
import { PROJECTS, GITHUB_PROJECTS } from '../constants';
            ...
            <div className="border-t border-border-color/15 pt-3 flex justify-between items-center">
              {project.link ? (
                <a 
                  ...
                  className="inline-flex items-center justify-center bg-text-main text-surface px-3 py-1.5 border border-border-color font-mono text-[10px] uppercase tracking-wider hover:bg-primary transition-colors focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
                ...
                <span className="inline-flex items-center gap-1.5 font-mono text-[10px] uppercase text-muted font-bold">
                  <ShieldAlert size={12} aria-hidden="true" /> Desktop Core System
                </span>
              )}
            </div>
          </div>
            ...
            {project.link && !isLiveStar && (
              <div className="flex border border-surface/20 bg-surface/5 rounded-sm p-0.5 overflow-hidden" role="tablist" aria-label="Media view toggle">
                <button
                  ...
                  className={`px-2 py-0.5 font-mono text-[9px] uppercase tracking-wider rounded-sm transition-all flex items-center gap-1 focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2 ${
...
const GitHubProjectsGrid: React.FC = () => {
  ...
  return (
    <section className="w-full max-w-[1200px] px-4 md:px-8 py-20 border-t border-border-color/20 flex flex-col gap-8 relative">
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-border-color pb-4">
        <div>
          ...
          <h2 className="font-display italic text-2xl md:text-4xl font-extrabold tracking-tight text-text-main">
            GitHub Core Engines
          </h2>
        </div>
      </div>
          ...
          return (
            ...
            >
              ...
              <div className="bg-text-main text-surface p-3 px-4 flex items-center justify-between border-b border-border-color">
                ...
                <span className="font-mono text-[9px] text-surface/50">
                  ENG // 0{idx + 1}
                </span>
              </div>

              {/* Image Preview */}
              <div className="h-44 border-b border-border-color bg-background-light relative overflow-hidden group shrink-0">
                <img 
                  src={imagePath} 
                  alt={project.title} 
                  className="w-full h-full object-cover object-top opacity-90 group-hover:scale-102 transition-transform duration-500" 
                />
                <div className="absolute inset-0 bg-gradient-to-t from-text-main/15 to-transparent pointer-events-none"></div>
              </div>

              {/* Card Body */}
              <div className="p-5 flex flex-col grow justify-between gap-4">
                <div className="flex flex-col gap-2">
                  <h3 className="font-sans font-black text-lg text-text-main leading-tight tracking-tight">
                    {project.title}
                  </h3>
                    ...
                    <div className="mt-1">
                      <ul className="list-none space-y-1">
                        {project.highlights.slice(0, 3).map((highlight, hIdx) => (
                          <li key={hIdx} className="flex items-center text-[10px] font-sans text-muted">
                ...
                <div className="flex flex-col gap-3">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {project.tech.slice(0, 4).map((tech, tIdx) => (
                      <span key={tIdx} className="font-mono text-[8px] px-1.5 py-0.5 border border-border-color/10 bg-background-light text-text-main">
                        {tech}
                      </span>
                    ))}
                    {project.tech.length > 4 && (
                      <span className="font-mono text-[8px] px-1.5 py-0.5 border border-border-color/10 bg-background-light text-muted">
                  ...
                  <div className="border-t border-border-color/15 pt-3 flex justify-between items-center">
                    <a 
                      ...
                      target="_blank" 
                      rel="noopener noreferrer"
                      className="w-full inline-flex items-center justify-center bg-surface hover:bg-text-main text-text-main hover:text-surface px-3 py-2 border border-border-color font-mono text-[10px] uppercase tracking-wider transition-colors focus-visible:outline-2 focus-visible:outline-primary focus-visible:outline-offset-2"
